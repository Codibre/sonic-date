...
import { differenceInDays } from 'src/methods';
import { getDstTransitions } from 'test/helpers/tz-offset-transitions';

describe(differenceInDays.name, () => {
	it('returns the number of full days between the given dates', () => {
		const result = new SonicDate(2012, 6 /* Jul */, 2, 18, 0).differenceInDays(
			new SonicDate(2011, 6 /* Jul */, 2, 6, 0),
		);
		assert(result === 366);
	});
	...
	it('returns a negative number if the time value of the first date is smaller', () => {
		const result = new SonicDate(2011, 6 /* Jul */, 2, 6, 0).differenceInDays(
			new SonicDate(2012, 6 /* Jul */, 2, 18, 0),
		);
		assert(result === -366);
	});
	...
	describe('edge cases', () => {
		it('the difference is less than a day, but the given dates are in different calendar days', () => {
			const result = new SonicDate(2014, 8 /* Sep */, 5, 0, 0).differenceInDays(
				new SonicDate(2014, 8 /* Sep */, 4, 23, 59),
			);
			assert(result === 0);
		});
		...
		it('the same for the swapped dates', () => {
			const result = new SonicDate(
				...
				23,
				59,
			).differenceInDays(new Date(2014, 8 /* Sep */, 5, 0, 0));
			assert(result === 0);
		});

		it('the time values of the given dates are the same', () => {
			const result = new SonicDate(2014, 8 /* Sep */, 6, 0, 0).differenceInDays(
				new SonicDate(2014, 8 /* Sep */, 5, 0, 0),
			);
			assert(result === 1);
		});
		...
		if (dstTransitions.start && dstTransitions.end) {
			...
			it(`works across DST start & end in local timezone: ${
				...
				// anchor to one hour before the boundary
				{
					...
					assert(sameTime(a, c));
					assert(sameTime(b, c));
					assert(c.differenceInDays(b) === 1); // normal 24-hour day
					assert(b.differenceInDays(a) === 1); // 23 hours -> 1 day
					assert(c.differenceInDays(a) === 2); // 47 hours -> 2 days
				}
				// anchor exactly, the boundary
				{
					...
					assert(sameTime(b, c));
					assert(c.differenceInDays(b) === 1); // normal 24-hour day
					assert(b.differenceInDays(a) === 1); // normal 24-hour day
					assert(c.differenceInDays(a) === 2); // 2 normal 24-hour days
				}
				...
				// until 25 hours have elapsed.
				{
					...
					assert(c.differenceInDays(b) === 1); // normal 24-hour day
					assert(b.differenceInDays(a) === 0); // 24.75 hours -> 0 full days (because hour lost to DST)
					assert(c.differenceInDays(a) === 1); // 48.75 hours -> 1 full day (because hour lost to DST)
				}
				// anchor to one hour before the boundary
				{
					...
					assert(sameTime(b, c));
					assert(c.differenceInDays(b) === 1); // normal 24-hour day
					assert(b.differenceInDays(a) === 1); // 25 hours -> 1 day
					assert(c.differenceInDays(a) === 2); // 49 hours -> 2 days
				}
	...
	it('returns NaN if the first date is `Invalid Date`', () => {
		const result = new SonicDate(NaN).differenceInDays(
	...
	it('returns NaN if the second date is `Invalid Date`', () => {
		const result = new SonicDate(2017, 0 /* Jan */, 1).differenceInDays(
			new SonicDate(NaN),
		);
		assert(isNaN(result));
	});
	...
	it('returns NaN if the both dates are `Invalid Date`', () => {
		const result = new SonicDate(NaN).differenceInDays(new SonicDate(NaN));
		assert(isNaN(result));
	});
});
